 ...
 */
public class ha extends SimpleChannelInboundHandler<String> {
...
//                写入数据库MongoDB的某一个集合，取出每一个id的缓冲队列的元素，单节点部署
                ...
                MongoCollection<Document> collectionmax = database1.getCollection(id + "#max");
...
//                collection.createIndex(Indexes.ascending("StartTime"));
                ...
                List<Document> documentsmax = new LinkedList<>();
                Document docMax = new Document();
                for (JSONObject jsonObject : jsonObjects) {
                    ...
                    if (dataArray != null && !dataArray.isEmpty()) {
                        ...
                        // 输出结果，不放进原始数据里面
                        docMax.append("max", max)
                                .append("min", min)
                                .append("avg", avg)
                                .append("ptp", max - min)
                                .append("Unit", jsonObject.get("Unit"))
                                .append("checkTime", jsonObject.get("checkTime"));


                        // 将 Document 添加到 documentsmax 列表
                        documentsmax.add(docMax);
                        docMax.clear();
                    }
